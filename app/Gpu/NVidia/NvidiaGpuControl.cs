...
public class NvidiaGpuControl : IGpuControl
{
    ...
    public int? GetGpuPower()
    {
        ...
        try
        {
            string output = NvidiaSmi.RunNvidiaSmiCommand("--query-gpu=power.draw --format=csv,noheader,nounits");
            if (float.TryParse(output.Trim(), System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out float power))
            {
                return (int)power;
            }
        }
        ...
        {
            Logger.WriteLine("GPU Power: " + ex.Message);
        }
