...
public static class HardwareControl
{
    ...
    public static int? cpuUse;

    // Network info (populated by NetworkControl)
    public static string wifiSSID => GHelper.Helpers.NetworkControl.WifiSSID;
    public static bool isOnline => GHelper.Helpers.NetworkControl.IsOnline;
    public static float downloadSpeed => GHelper.Helpers.NetworkControl.DownloadSpeed;
    public static float uploadSpeed => GHelper.Helpers.NetworkControl.UploadSpeed;

    public static float? cpuPower;
    static uint lastEnergy;
    static long lastPowerUpdate;
    static double energyUnit = -1;

    static Ryzen.Ols ols = new Ryzen.Ols();

    public static void GetIntelPackagePower()
    {
        if (ols.GetStatus() != (uint)Ryzen.Ols.Status.NO_ERROR)
        {
            return;
        }
        ...
        try
        {
            ...
            {
                uint eax = 0, edx = 0;
                if (ols.Rdmsr(0x606, ref eax, ref edx) != 0)
                {
                    energyUnit = Math.Pow(0.5, (double)((eax >> 8) & 0x1F));
                }
            }

            if (energyUnit <= 0) return;

            uint eaxCurrent = 0, edxCurrent = 0;
            if (ols.Rdmsr(0x611, ref eaxCurrent, ref edxCurrent) != 0)
            {
                long currentT = DateTimeOffset.Now.ToUnixTimeMilliseconds();
                if (lastPowerUpdate > 0 && currentT > lastPowerUpdate)
                {
                    uint diffEnergy = eaxCurrent - lastEnergy;
                    cpuPower = (float)(diffEnergy * energyUnit * 1000 / (currentT - lastPowerUpdate));
                }
                lastEnergy = eaxCurrent;
                lastPowerUpdate = currentT;
            }
        }
        catch (Exception ex)
        {
            Debug.WriteLine("Intel Power Reading: " + ex.Message);
        }
    }
    ...
    public static void ReadSensors(bool log = false)
    {
        ...
        GetIntelPackagePower();
        GHelper.Helpers.NetworkControl.Refresh();
